...
async def test_opencode_shell_enrichment_retries_vendor_read_model_race(tmp_path):
    transport = MemoryHttpTransport()
    registry = AdapterRegistry()
    adapter = registry.opencode
    adapter.attach_transport(transport)
    ...
    async def delayed_exit(method, path, *, json=None):
        nonlocal calls
        calls += 1
        metadata = {} if calls == 1 else {"exit": 1}
        return [{
            "info": {"id": "assistant"},
            "parts": [{
                "type": "tool",
                ...
                "state": {
                    "status": "completed",
                    "output": "FAILED test_csv.py::test_rows\n1 failed",
                    "metadata": metadata,
                },
            }],
        }]

    transport.request = delayed_exit
