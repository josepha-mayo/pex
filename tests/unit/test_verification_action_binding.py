...
def binding():
    ...
    goal = Goal(
        ...
        title="Verify tests",
        objective="Pass the public test suite",
        created_at=now,
        updated_at=now,
    )
    event = HarnessEvent(
...
def proposal(request, payload, kind="REQUEST_VERIFICATION"):
    return _action_from_proposal(
        request,
        {
            "type": kind,
            "payload": payload,
            "rationale": "The test result is unobserved.",
...
def test_actual_parser_roundtrip_binds_only_local_authority(binding, representation):
    ...
    if representation == "reference":
        ...
        payload = {
            ...
            else _redact_payload_value(request, canonical)
        }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("goal_id", "other-goal"),
        ("request_event_id", "other-stop"),
        ("cwd", "C:/other"),
        ("project_id", "other-project"),
        ("harness_type", "cursor"),
        ("relative_targets", ["different.py"]),
        ("timeout_seconds", 61),
...
def test_valid_probe_does_not_authorize_model_supplied_commands(binding, representation):
    request, gathering = binding
    payload = (
        {"probe_id": "probe-one", "kind": "pytest"}
        if representation == "reference"
...
def test_unittest_probe_binds_bridge_owned_exact_target_copy(binding):
    request, gathering = binding
    gathering.probe = VerificationProbe.model_validate(
        {
            **gathering.probe.model_dump(mode="json"),
    ...
    action = proposal(
        request,
        {"probe_id": "probe-one", "kind": "python_unittest"},
    )

    bound = bind_verification_action(action, gathering, request)

    assert bound is not None
    assert bound.payload["probe"]["kind"] == "python_unittest"
    assert "python -m unittest" in bound.payload["text"]
    assert "test_timeline.py" in bound.payload["text"]
