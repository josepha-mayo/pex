"""Live Cursor stop hook through Pipeline inspect. Does not spawn another Cursor."""
...
@pytest.mark.asyncio
async def test_live_cursor_incomplete_stop_sends_specific_followup(tmp_path: Path):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            start = await client.post(
                ...
                json={
                    ...
                    "conversation_id": "live-cursor-inspect",
                    "workspace_roots": [str(tmp_path)],
                },
            )
            assert start.status_code == 200
            attached = await client.post(
                "/v1/sessions/cursor:live-cursor-inspect/attach",
            ...
            body = stop.json()
            followup = str(body.get("followup_message") or "")
            proof["followup"] = followup
            proof["hook_body"] = body
            rows = await store.list_interventions("cursor:live-cursor-inspect")
            ...
            (scratch / "cursor_inspect_proof.json").write_text(
            ...
            }, f"incomplete Cursor stop stayed silent: {proof!r}"
            assert followup
