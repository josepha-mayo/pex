"""Live Kimi ACP idle → Pipeline inspect. Does not spawn `kimi`."""
...
@pytest.mark.asyncio
async def test_live_kimi_idle_stop_sends_specific_prompt(tmp_path: Path):
    ...
    session = registry.kimi.ingest_hook({"session_id": "live-kimi-inspect", "cwd": str(tmp_path)})
    ...
    try:
        ...
        }, f"incomplete Kimi idle stayed silent: {proof!r}"
