"""Live supervisor + simulated Kimi ACP prompt-result inspection.

This does not spawn Kimi and is not provider-live evidence.
"""
...
@pytest.mark.asyncio
async def test_live_kimi_prompt_result_sends_specific_prompt(tmp_path: Path):
    ...
    transport.sessions = [{"sessionId": "live-kimi-inspect", "cwd": str(tmp_path)}]
    registry = AdapterRegistry()
    registry.kimi.attach_acp(transport)
    ...
    session = (await registry.kimi.discover_sessions())[0]
    session.goal_id = goal.id
    session.cwd = str(tmp_path)
    await store.upsert_session(session)
    proof: dict = {"used_llm": False, "action_taken": None, "prompts": []}
    pump = registry.kimi.start_pipeline_pump(pipeline.ingest_event)
    try:
        transport.events.append(
            {
                "method": "session/update",
                "params": {
                    "sessionId": "live-kimi-inspect",
                    "update": {
        ...
        assert await registry.kimi.send_message(session, "Inspect whether the report is complete.")
        for _ in range(120):
        ...
        (scratch / "kimi_inspect_proof.json").write_text(
        ...
        }, f"incomplete simulated Kimi prompt result stayed silent: {proof!r}"
