...
async def test_live_opencode_idle_stop_sends_specific_prompt(tmp_path: Path):
    ...
    session = registry.opencode.ingest_hook(
        {"session_id": "live-opencode-inspect", "cwd": str(tmp_path)}
    )
    session.goal_id = goal.id
    session.cwd = str(tmp_path)
    await store.upsert_session(session)
    proof: dict = {"used_llm": False, "action_taken": None, "prompts": []}
    pump = registry.opencode.start_pipeline_pump(pipeline.ingest_event)
    try:
        transport.events.append(
            {
                "id": "evt_idle",
                "type": "session.idle",
                "properties": {"sessionID": "live-opencode-inspect", "cwd": str(tmp_path)},
                "text": "I am done.",
            }
        )
        for _ in range(120):
        ...
        (scratch / "opencode_inspect_proof.json").write_text(
        ...
        }, f"incomplete OpenCode idle stayed silent: {proof!r}"
        assert proof["prompts"], f"prompt_async was not called: {proof!r}"
