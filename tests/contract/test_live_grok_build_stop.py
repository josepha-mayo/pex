"""Live Grok Build Stop hook through Pipeline inspect. Does not spawn `grok`."""
...
@pytest.mark.asyncio
async def test_live_grok_build_incomplete_stop_sends_specific_nudge(tmp_path: Path):
    ...
    try:
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            attached = await client.post(
                "/v1/sessions/grok_build:live-grok-inspect/attach",
            ...
            followup = str(inbox[-1] if inbox else "")
            proof["followup"] = followup
            proof["hook_body"] = body
            rows = await store.list_interventions("grok_build:live-grok-inspect")
            ...
            }, f"incomplete Grok Build stop stayed silent: {proof!r}"
            assert followup
