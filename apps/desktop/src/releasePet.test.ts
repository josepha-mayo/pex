...
test("all pet surfaces retain sprite frames without extra continuous transforms or filters", async () => {
  ...
  assert.match(
    styles,
    /\.sprite-3d\s*\{[^}]*animation:\s*none;[^}]*filter:\s*none;/u,
    "the Home, picker and overlay sprites must avoid redundant effects",
  );
  assert.doesNotMatch(styles, /@keyframes pet-(?:breathe|listen)|animation:\s*pet-(?:breathe|listen)/u);
  assert.match(
    atlas,
