...
`release_ready` field remains false solely because `native_runtime_proof` is false. Preserve
that distinction: package integrity is green, but PEX was intentionally not opened after the
reported whole-PC freeze. Fresh native startup/idle measurements, message/pet interaction,
and normal-size playback/transparency remain open.

Commit `d66e6a1` makes pet-message dismissal persist across restart without hiding the pet.
...
View/read cancellation is pushed as 70ba867. Cursor inbox slice 43d9337 bounds
actual file/marker reads and each record batch, and stops erasing unread oversized
backlogs. The follow-up now reads without consuming, performs file work off-loop,
and checkpoints only after durable observer ingestion. HTTP fail-open results and
event-id collision errors never imply acknowledgement. Opened-descriptor, source
...
- `BUNDLE_MANIFEST.md` lists 19 payload files, including the current architecture diagram and
  tracked live OpenCode recovery plus quiet-completion Markdown/JSON receipts. A
  fresh verifier recomputed every byte count and SHA-256 with zero missing files and zero
  mismatches; the directory contains 20 files including the manifest.
- `BUNDLE_MANIFEST.md` SHA-256 is
  `7aaf701ad5911cb2e10d31011982fc177bec5a9cc3a96d2de75a08179dd3b67e`.
