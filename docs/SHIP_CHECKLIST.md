...
| Critical backend repairs | IN PROGRESS | Cursor preparation/flush separation, generic/OpenCode outcome attribution and bounded supervisor context are pushed with local regression evidence. Next: retain exact tool observations separately for main and verifier, then prove the real same-session supervisor loop. |
