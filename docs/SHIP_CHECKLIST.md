...
| Critical backend repairs | IN PROGRESS | Cursor preparation/flush separation is pushed. Generic/OpenCode outcome attribution and bounded supervisor context have local regression evidence; finish their reviewed checkpoint. Next: retain exact tool observations separately for main and verifier, then prove the real same-session supervisor loop. |
| Human-facing product | IN PROGRESS | Canonical loading/stale/failure states have local checks. Next: usable first-run connection and visible recoverable startup, followed by actual primary-flow UI-to-backend verification. |
...
| 1 | LOCALLY VERIFIED SOURCE, CHECKPOINT IN PROGRESS: generic outcomes fail closed and OpenCode retains exact assistant parent lineage. | Unrelated human prompt, old turn, missing parent, uncertain delivery and scope mismatch regressions; exact parent-bound terminal result; unsupported causal proof stays unknown. Finish final combined review/push; live run remains open. |
| 2 | LOCALLY VERIFIED SOURCE, CHECKPOINT IN PROGRESS: bounded durable context/decisions reach the frozen supervisor request and paged evidence tools. | Sibling evidence/rejected approach available with provenance; private/foreign/stale material excluded; offered IDs and packet hash retained; replay uses original packet. Offered does not mean model-used; exact tool observations remain task 3. |
