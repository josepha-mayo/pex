...
These are the immediate steps within stage 1, not another competing product plan. The prefix-retention repair is already reviewed/pushed as `7570297`, with documentation checkpoint `458c2c8` and the 413-case integration receipt in the handoff. Do not repeat that completed repair or mistake it for installed-runtime proof. The current cycle is workspace authority; new helper files are uncommitted WIP until main integration and independent review pass.

1. [ ] **Refresh authority and reproduce the remaining attachment failures — main.** Reread the three specs/current handoff and relevant source before implementation; reconcile file ownership. Preserve regressions for two distinct machine origins sharing a path, the typed-project raw-cwd shortcut, stale bindings and directory replacement. Record exact audit scope; a changed-path review is not a full-file audit.
2. [ ] **Review server-measured directory identity — `transport_review` implementation; main acceptance.** Own only `local_workspace.py` and `test_local_workspace.py`. Agent reports 39 passed/1 environment-dependent symlink skip and Ruff clean; main has not yet independently accepted or integrated this checkpoint. Check valid aliases and negative/race cases. Sampled filesystem identity is not a hardware attestation, atomic lock or proof of the worker's retained cwd handle.
3. [ ] **Finish explicit local-origin configuration — `legacy_attach_fence` implementation; main acceptance.** Own only `local_origin_config.py` and `test_local_origin_config.py`. Require strict bounded loading, exact revision/choice-ID compare-and-swap, server measurement and explicit storage-rebind confirmation. Corrupt files must not silently become unconfigured or be overwritten. Never guess an origin, silently relabel existing project history, or trust caller-supplied proof as measured evidence. Module is WIP; no integrated result is claimed.
4. [ ] **Integrate workspace authority end-to-end — main.** Bind inspect/confirm to the explicit origin, exact active project locator and measured workspace. Revalidate after reconciliation and in Store publication against current immutable project authority. Test expiry, replacement, configuration change, stale confirmation, cancellation and valid recovery. No production origin choice or global configuration change is authorized by writing this plan.
...
6. [ ] **Independently review, test and publish this repair — main plus a reviewer who did not own the patch.** Review every changed path and positive/negative integration cases; run all affected compatibility files and relevant frontend checks. Record failures/skips honestly. Update audit coverage, handoff and known failures, then commit/push only accepted owned paths and verify exact remote hash. Leave unowned `loop.py` and unfinished agent work unstaged.
...
- **Main:** workspace-authority API/attachment/Store integration, acceptance of the two helper modules, full-spec audit and final evidence acceptance; later runtime/UI verification. No integration source changes are claimed by this planning checkpoint.
- **`transport_review`:** owns only `services/bridge/src/pex_bridge/local_workspace.py` and `tests/unit/test_local_workspace.py`; implementation reported ready for independent review, not yet accepted or committed.
- **`attachment_review`:** completed read-only origin/migration and desktop-connection design; available for bounded independent review or a precisely assigned UI slice after the API contract and applicable skills are reviewed. No current UI edit ownership.
- **`legacy_attach_fence`:** owns only `services/bridge/src/pex_bridge/local_origin_config.py` and `tests/unit/test_local_origin_config.py`; strict config/load/save and focused tests are in progress, not integrated evidence.
