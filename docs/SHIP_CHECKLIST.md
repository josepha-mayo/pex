...
- [ ] **Finish the current credential-settings repair (main).** Complete the in-progress `supervisorDraft.ts` / `App.tsx` wiring: bind a pasted key to its chosen credential destination, clear it when that destination changes, guard duplicate saves and stale responses, and disable conflicting edits during a save. Add regressions, run desktop tests/typecheck, and obtain independent diff review. The draft is currently incomplete and must not be shipped or counted as a working fix.
...
- [ ] **Repair first-run instructions and installer (setup reviewer, main integration).** Check interpreter selection after `uv sync`, native-command failure handling, global hook side effects, missing sidecar requirements, and the documented nonexistent Attach control. Align instructions with the actual supported UI/CLI; validate without changing installed hooks or restarting the user's running bridge.
- [ ] **Integrate and push the bounded repair checkpoint (main).** Review all changed lines, run proportional combined gates, update audit coverage/handoff, stage only reviewed files, push, and verify the remote commit. Preserve the unexplained `loop.py` edit outside the checkpoint.
...
| 6 | PARTIAL LOCAL SOURCE: startup now shows safe recovery, serialized retry and single-instance handling. Setup/docs/demo/package remain open. Installer uses plain Python after `uv sync` and references a nonexistent Attach control; recorder may accept an unauthenticated offline UI. | 74 frontend checks/TypeScript and 12 Rust tests pass; Rust needed a test-only externalBin override because frozen cursor-observe is missing. Still require normal release config, packaged bootstrap/port/crash/retry smoke, synced interpreter, real attachment flow, authenticated source-valid demo and exact package hashes. |
