...
These are the immediate steps within stage 1, not another competing product plan. Prefix-retention repair follows source `8a402c2` and plan `0bdb6b8`; the current handoff records its independent review and verification/push state. No installed-runtime proof is implied.

1. [x] **Refresh the audit inputs — main.** All three specs and changed source/test paths read for the prefix repair. Worktree ownership reconciled; the unowned `loop.py` diff remains untouched. This is not whole-repository review.
2. [x] **Capture prefix-loss regressions — main; independent reviewer: `transport_review`.** Five initial failures reproduced lost observations through coordinator, adapter, Pipeline and real temporary Store. Independent review additionally caught initial reconciliation capacity loss.
3. [x] **Repair observation retention — main.** Validated prefixes retain original identity/order and frozen per-event state. Status/flags validation precedes acceptance; invalid/foreign suffixes are excluded. Dedicated record-only recovery has retry, duplicate, cancellation, full-queue, Store failure and replacement-owner tests; it does not authorize post-loss semantic processing or establish complete raw/crash coverage.
...
5. [x] **Independently review and integrate — main plus bounded reviewers.** Actual diff, negative/positive cases and full affected compatibility files reviewed; final 413 tests passed across 20 files. Earlier fixture and capacity failures are disclosed in the handoff. No whole-repository approval.
6. [x] **Publish the verified prefix checkpoint — main.** Ten reviewed source/test paths pushed as `7570297`; exact remote hash verified. Audit/handoff/known-failure receipts updated. Next: the still-open workspace-authority item, remaining raw/crash coverage, safe same-worker control and real supervisor loop. All seven product stages remain required.
