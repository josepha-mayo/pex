...
Follow these steps in order; the seven stages below define their full acceptance criteria. Accepted source baseline is `03045b5`, including connection UI `cd39913`, cleanup `c15a2fc` and continuity `c0db453`. Main owns integration; bounded owners and independent reviewers must have explicit non-overlapping assignments. No agent currently owns UI edits.
...
3. [ ] **Close observation and same-worker control gaps.** Internal same-channel start/steer primitive and shared cleanup are accepted in `03045b5`, main 406-pass gate. Complete required durable received-envelope journal before filtering/clearing; fix fresh input/turn reconciliation and PEX-message provenance; wire durable goal/policy/effect dispatch and truthful capabilities. Resolve protected installed-runtime prerequisites. Record unavoidable external-client post-check races; do not claim server CAS that does not exist. No substitute worker or weakened protections.
...
- [ ] Journal received envelopes durably before filtering/clearing; reject new control if the journal fails. Preserve raw failure provenance without replaying stale state as live.
