...
| Runs consistently on its intended platform | Exact-source RC16 passed Windows and Ubuntu source suites, desktop builds, native packaging, bootstrap tests, and packaged bridge smoke checks. | Satisfied for RC16 |
...
| Free judge-accessible working demo or test build through judging | RC16 is public and free at the repository release URL. | Satisfied for RC16 |
...
2. Repeat the bounded live recovery only if sponsor-provided credit is available
   with a verified cutoff; RC16 packaging made zero paid provider calls.
