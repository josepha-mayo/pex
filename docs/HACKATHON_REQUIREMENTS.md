...
| Runs consistently on its intended platform | Exact-source RC21 passed Windows and Ubuntu source suites, desktop builds, native packaging, bootstrap tests, and packaged bridge smoke checks. | Satisfied for RC21 |
...
| Free judge-accessible working demo or test build through judging | RC21 is public and free at the repository release URL. | Satisfied for RC21 |
...
2. Repeat the bounded live recovery only if sponsor-provided credit is available
   with a verified cutoff; RC21 packaging made zero paid provider calls.
