...
replacement agent with lost context. In the fresh release-source proof, that
single correction produced both exact artifacts, PEX observed the supported
outcome, and then stayed quiet. The audit keeps every decision tied to the
specific goal, evidence, worker session, delivery, and observed result.”
